...
# - 输出键维 `D = bondD(alg.trunc)`；
# - `D ≥ 朴素构造键维` 时短路返回精确结果（不压缩）。
#
...
"add/hadamard 的结果装配（MPS）：`D ≥ 朴素构造键维` 时短路返回精确结果，
否则压缩到键维 `D`。"
function _algebra_result(K::Vector{<:Array{T,3}}, D::Int, alg::Algorithm) where {T}
...
"add/hadamard 的结果装配（MPO）：`D ≥ 朴素键维` 时精确，否则压缩。输出取环迹
对齐标量 `c` 乘左规范张量 `AL`：输出幅值 `c^N·tr(∏AL)` 精确恢复目标幅值
（含相位、任意 `D`）。压缩收敛态的逐 site 相位是本征解的规范自由度（MPSKit
同样不钉定本征解相位），环迹意义下幅值已对齐。不能收集 `x.AC`：
`AC = AL·C` 在键间插入 `C` 加权，其周期 trace 是规范依赖的 C 加权量而非算符幅值。"
function _mpo_algebra_result(K4::Vector{<:Array{T,4}}, D::Int, alg::Algorithm) where {T}
...
"""
    add(ψ₁, ψ₂; alg = VOMPS()) -> InfiniteCanonicalMPS
    add(W₁, W₂; alg = VOMPS()) -> InfiniteMPO
...
加权、改变射线，不能用）。输出键维 `D = bondD(alg.trunc)`；朴素构造键维
`≤ D` 时短路返回精确结果，否则变分压缩（`alg` 支持 `VOMPS()` 与 `IDMRG()`）。
"""
...
function add(W1::InfiniteCanonicalMPO, W2::InfiniteCanonicalMPO;
    ...
    return _mpo_algebra_result(K4, bondD(alg.trunc), alg)
end
