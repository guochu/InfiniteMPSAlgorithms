...
#   验证精确可加 / 可乘 / 外积结构；
# - add / hadamard：朴素精确构造（朴素键维 ≤ trunc.D 时短路精确）对标 MPSKit 语义
#   （键维直和 / fuse_mul_mpo）；
...
@testset "exact_add：周期 trace 精确可加（debug 基准）" begin
    ...
    # 长度不匹配抛错
    @test_throws DimensionMismatch exact_add(ψ1.AL, randommps(T, [2, 2, 2], 3).AL)
end
...
@testset "add：MPS 加法（朴素精确，对标 MPSKit）" begin
    T = ComplexF64
    Random.seed!(42)
    ψu = prodmps(T, [2, 2], [1, 1])   # |00⟩
    ψd = prodmps(T, [2, 2], [2, 2])   # |11⟩
    s = add(ψu, ψd)
    ...
    # 长度 / 逐 site 物理维不匹配
    @test_throws DimensionMismatch add(ψu, randommps(T, [2, 2, 2], 2))
    @test_throws DimensionMismatch add(ψu, randommps(T, [3, 2], 2))
end
...
@testset "add：MPS 加法压缩（VOMPS 与 IDMRG）" begin
    ...
    for alg in (VOMPS(trunc = truncdim(3), maxiter = 200), IDMRG(trunc = truncdim(3), maxiter = 200))
        Random.seed!(1)
        s2 = add(ψ1, ψ1; alg = alg)             # ψ1 + ψ1 = 2ψ1（键 6 → 3 无损）
...
@testset "add：MPO 加法（朴素精确 + 压缩）" begin
    ...
    tgt = 2 .* dI
    for alg in (VOMPS(trunc = truncdim(1), maxiter = 200), IDMRG(trunc = truncdim(1), maxiter = 200))
        Random.seed!(1)
        s2 = add(I2, I2; alg = alg)
        @test bonddim(s2, 1) == 1
...
@testset "mult：MPO 乘法（朴素精确对照 + 压缩）" begin
    ...
    @test reshape(_dense_mpo_repr(InfiniteMPO(P)), 4, 4) ≈
          reshape(_dense_mpo_repr(W1), 4, 4) * reshape(_dense_mpo_repr(W2), 4, 4) atol = 1e-10
    # 迭代乘法：W1·I = W1（trunc.D = 2 = 目标键维）。与目标平行即可（整体相位/尺度是
    # 输出射线规范的自由度）
    P1, ov1 = mult(W1, I2; alg = VOMPS(trunc = truncdim(2)))
...
@testset "hadamard：element-wise 乘积（物理维不变）" begin
    ...
    Random.seed!(45)
    ψ1 = randommps(T, [2, 2], 3)
    ψ2 = randommps(T, [2, 2], 3)
    c1 = _dense_mps_repr(ψ1)
    c2 = _dense_mps_repr(ψ2)

    # 精确路径（默认 trunc.D = 64 ≥ 朴素键维 9 → 短路精确）：波形 ∝ 逐点乘积
    # c1 .* c2，物理维不变、键维 = 3·3。
    ...
    # 压缩路径（两算法）：trunc.D = 9 = 精确键维 → 无损，与 exact 平行
    for alg in (VOMPS(trunc = truncdim(9), maxiter = 200), IDMRG(trunc = truncdim(9), maxiter = 200))
        Hc = hadamard(ψ1, ψ2; alg = alg)
        @test max_bonddim(Hc) == 9
        @test abs(dot(Hc, H12)) > 1 - 1e-8
    end

    # 长度 / 逐 site 物理维不匹配
    @test_throws DimensionMismatch hadamard(ψ1, randommps(T, [2, 2, 2], 2))
    @test_throws DimensionMismatch hadamard(ψ1, randommps(T, [3, 2], 3))
end
