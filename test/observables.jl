@testset "观测量" begin
    ...
    @test real(expectation_value(ρ, (1,) => Z)) ≈ 1 atol = 1e-12
    @test real(expectation_value(ρ, (1, 2) => kron(Z, Z))) ≈ 1 atol = 1e-12
    ...
    ρn = product_mps(T, [2, 2], [1, 2])   # |↑↓⟩
    @test real(expectation_value(ρn, (1,) => Z)) ≈ 1 atol = 1e-12
    @test real(expectation_value(ρn, (2,) => Z)) ≈ -1 atol = 1e-12
    @test real(expectation_value(ρn, (1, 2) => kron(Z, Z))) ≈ -1 atol = 1e-12
    ...
    Random.seed!(1)
    ψ, envs, _ = find_groundstate(random_mps(T, [2, 2], 12), Hm,
                                  VUMPS(maxiter = 200, tol = 1e-9))
    @test abs(expectation_value(ψ, (1,) => Sz(T)^2) - 0.25) < 1e-12
