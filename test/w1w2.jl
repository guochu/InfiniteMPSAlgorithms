@testset "W^I / W^II 时间演化" begin
    ...
    bulk = Hxxz.bulk
    dt = 0.01
    ψ0 = random_mps(T, [2, 2], 8)
    e0 = real(expectation_value(ψ0, HJ) / 2)

    # 恒等哈密顿量：WI 演化 MPO = 恒等，apply 后状态不变
    empty_bulk = bulk_mpo(zeros(T, 2, 2), Tuple{Float64,Matrix{T},Matrix{T}}[])
    Ievo = make_time_mpo(empty_bulk, dt, WI())
    out = apply(ψ0, Ievo)
    @test abs(real(expectation_value(out[1], HJ) / 2) - e0) < 1e-10
    ...
    out2ψ, out2ov = apply(ψ0, W2)
    @test abs(real(expectation_value(out2ψ, HJ) / 2) - e0) < 1e-4
    ...
    out1ψ, _ = apply(ψ0, W1)
    err1 = abs(real(expectation_value(out1ψ, HJ) / 2) - e0)
    @test err1 < 1e-4

    # make_time_mpo 直接接受 Jordan 哈密顿量；阈值同上（MPO 自身误差）
    W2h = make_time_mpo(HJ, dt, WII())
    out3ψ, _ = apply(ψ0, W2h)
    @test abs(real(expectation_value(out3ψ, HJ) / 2) - e0) < 1e-4
